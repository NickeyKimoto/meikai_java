...
public class ShapeTester {
	...
	*/
	...
	public static void main(String[] args) {
		...
		final int MAXIMUM_NUMBER_OF_SHAPE_TYPES = 9;
		...
		for (int i = 0; i < outputShapes.length; i++) {
			...
			// 表示させる図形の種類の入力を促す
			System.out.print(++shapeNumber + "番の図形の種類(1…点／2…水平直線／3…垂直直線／4…長方形"+ "／5…左下直角の二等辺三角形／\n"
					+ "6…左上直角の二等辺三角形／7…右下直角の二等辺三角形／8…右上直角の二等辺三角形／9…平行四辺形)：");
			// 入力された値を読み込む
			...
			// 9が入力された場合
			} else if (shapeType == NUMBER_OF_SHAPE_TYPES[8]) {
		...
		for (int i = 0; i < outputShapes.length; i++) {
				...
				// もしこの要素がPlane2D型であれば
				if (outputShapes[i] instanceof Plane2D) {
					// 図形の面積を表示する
					System.out.print("Area:[" + ((Plane2D) outputShapes[i]).getArea() + "]");
				}
				// 改行する
				System.out.println();
